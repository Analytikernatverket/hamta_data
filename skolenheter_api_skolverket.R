...
hamta_skolenheter_api_skolverket <- function(
    region_vekt = "20",       # länskoder = hämtar alla kommuner i det länet, går att skicka både länskoder och kommunkoder
  ...
  hamta_skolenhetsinfo <- function(skolenhetskod) {
    ...
    geo   <- if (nrow(besok) > 0) if(any(str_detect(names(besok), "geoCoordinates"))) TRUE else FALSE
  ...
  skolor_url <- glue("{skolenheter_basurl}?status={paste0(skolstatus, collapse = ',')}&municipality_code={paste0(kommunkoder, collapse = ',')}") 
  ...
  skolor_df <- map(skolenhetskoder, ~ hamta_skolenhetsinfo(.x)) %>% 
    list_rbind()
  ...
  if (tolower(geografi) == "sweref99") {
    ...
    skolor_df <- skolor_df %>% 
      mutate(
        geom = pmap(
          ...
          ~ if (is.na(..1) || is.na(..2)) st_geometrycollection()
          else st_point(c(as.numeric(..1), as.numeric(..2)))
        )
      ) %>%
    ...
    skolor_df <- skolor_df %>% 
      mutate(
        geom = pmap(
          list(Koordinat_WGS84_Lat, Koordinat_WGS84_Lon),
          ~ if (is.na(..1) || is.na(..2)) {
            st_geometrycollection()              # tom geometri
          } else {
            st_point(c(as.numeric(..1), as.numeric(..2)))
          }
        )
      ) %>%
