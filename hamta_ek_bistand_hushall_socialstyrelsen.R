...
hamta_ek_bistand_hushall_socialstyrelsen <- function(
    headless = TRUE,
    timeout = 30,
    verbose = TRUE
) {
  ...
  sess |> hitta("#AR") |> selenider::elem_select(value = alla_ar)
  ...
  inlasfil <- suppressMessages(read_xlsx(fil, col_names = FALSE))
